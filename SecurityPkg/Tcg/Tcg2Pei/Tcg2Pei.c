  ...
  )
{
  ...
  // [TSec]
  // Check if support TPM 2.0 device or not?
  // PcdTpmInstanceGuid should be set in SecurityPkg.dec.
  if (CompareGuid (PcdGetPtr(PcdTpmInstanceGuid), &gEfiTpmDeviceInstanceNoneGuid) ||   // indicate TPM is disabled
      CompareGuid (PcdGetPtr(PcdTpmInstanceGuid), &gEfiTpmDeviceInstanceTpm12Guid)){   // indicate TPM 1.2 is supported
    DEBUG ((DEBUG_INFO, "No TPM2 instance required!\n"));
    return EFI_UNSUPPORTED;
  }

  // [TSec] gTpmErrorHobGuid, HOB GUID used to record TPM device error.
  // If in S3 path, TpmErrorHob maybe not NULL.
  if (GetFirstGuidHob (&gTpmErrorHobGuid) != NULL) {  
  ...
  if (BootMode != BOOT_ON_S3_RESUME) {
    ...
    Status = (**PeiServices).RegisterForShadow(FileHandle); // [TSec] what dose this function do ???
